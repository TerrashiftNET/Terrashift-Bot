...
const mods = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../mods.json"), "utf8"),
);
...
class UserCommand extends Command {
  ...
  async chatInputRun(interaction) {
        ...
        if (mod.source === "curseforge") {
          try {
            ...
            if (addon.latestFiles[0].gameVersions[0].includes(version)) {
              mod_status.push(
                `✅ - Mod ${addon.name} has a ${version} version`,
              );
            } else {
              mod_status.push(
          ...
          try {
            ...
            if (data.game_versions.includes(version)) {
              mod_status.push(
                `✅ - Mod ${data.title} has a ${version} version`,
              );
            } else {
              mod_status.push(
    ...
    fs.writeFileSync(
      path.join(__dirname, "../1.21.json"),
      JSON.stringify(mod_status, null, 2),
    );
